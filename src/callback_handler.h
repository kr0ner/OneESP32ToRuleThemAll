...
class CallbackHandler {
   public:
    void addCallback(const std::pair<CanMember, PropertyType> key, std::function<void(const SimpleVariant&)> value) {
        callbacks.insert({std::move(key), std::move(value)});
        //add property to list so it can be identified later by address
        PropertyList::instance().addProperty(key.second);
    }

    std::function<void(const SimpleVariant&)> getCallback(const std::pair<CanMember, PropertyType> key) const {
        auto it = callbacks.find(key);
        if (it != callbacks.end()) {
            return it->second;
        }
        ESP_LOGW("CallbackHandler", "Callback not found for %s 0x%04x", key.first.name.c_str(), key.second);
   ...
   private:
    CallbackHandler() {}
    std::map<std::pair<CanMember, PropertyType>, std::function<void(const SimpleVariant&)>> callbacks;
};
